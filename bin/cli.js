...
if (!pnpmExist) {
  console.log(
    "\x1b[31m%s\x1b[0m",
    `----- Pnpm does not exist. Installing pnpm`
  );
...
console.log("\x1b[34m%s\x1b[0m", `----- Installing all Dependancies`);
const installFDeps1 = runCommand(
  `cd ${repoName} && pnpm install -D @testing-library/jest-dom @testing-library/react @types/node @types/react @types/react-dom @typescript-eslint/eslint-plugin @typescript-eslint/parser autoprefixer eslint eslint-config-next eslint-config-prettier jest jest-environment-jsdom postcss prettier tailwindcss typescript @mui/material @emotion/react @emotion/styled`
);
const installFDeps2 = runCommand(
  `cd ${repoName} && pnpm install daisyui ethers next next-seo notistack react react-dom web3modal`
);
